...
class ProductListCreateViewTestCase(APITestCase):
    ...
    def test_product_creation(self):
        ...
        self.assertDictContainsSubset(
            DEMO_PRODUCT, json.loads(response.content)
        )
    ...
    def test_product_list_view(self):
        ...
        response = self.client.get(url, format="json")
        self.assertDictContainsSubset(
            DEMO_PRODUCT, json.loads(response.content)[0]
        )
    ...
    def test_duplicate_product_creation(self):
        ...
        duplicate_attempt_response = self.client.post(
            url, DEMO_PRODUCT, format="json"
        )
        self.assertEqual(
            duplicate_attempt_response.status_code, status.HTTP_409_CONFLICT
        )
